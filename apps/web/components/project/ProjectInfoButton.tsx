...
export function ProjectInfoButton({ project }: { project: Project }) {
  ...
  return (
    <>
      ...
      <Sheet open={open} onClose={() => setOpen(false)} title={project.title}>
        {(() => {
          const story = project.blocks.find((b) => b.type === "story");
          const text = story ? story.data.text : project.subtitle;
          return text ? <p className="text-base text-ink-muted">{text}</p> : null;
        })()}
      </Sheet>
    </>
  );
